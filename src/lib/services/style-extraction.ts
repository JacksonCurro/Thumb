...

{
  ...
  "rawDescriptors": "A detailed 3-4 sentence visual description covering: subject matter, background treatment, lighting style, colour grading approach, text styling details, graphic overlays, and overall composition strategy. Be specific about colours, contrast levels, and spatial relationships."
}
...
- "rawDescriptors": must be specific enough that another designer could recreate the visual style without seeing the original. Include exact colour relationships, contrast ratios (high/medium/low), lighting direction, and text treatment details.
...
export async function extractStyleFromImage(
  ...
  // Validate structure
  if (!parsed.palette?.dominant?.length || !parsed.layout?.subjectPosition || !parsed.moodTags?.length) {
